...
export class MainScene extends g.Scene {
	...
	public setSaitan: (n: number) => void;
	public setTekazu: (n: number) => void;
	...
	constructor(param: g.SceneParameterObject) {
		param.assetIds = [
			"img_numbers_n", "img_numbers_n_red", "title", "start", "finish", "score", "time","disk","bar","base","label1",
			"waku", "wakuf", "clear", "waku", "select", "cursor",
			"config", "volume", "test", "glyph72", "number_w", "number_b", "number_p",
			"se_start", "se_timeup", "bgm", "se_clear", "se_move", "se_finish", "se_miss"];
		super(param);
		...
		this.loaded.add(() => {
			...
			//手数
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["score"], x: 505, y: 85, height: 32, srcY: 64 }));
			const labelTekazu = new g.Label({
				scene: this,
				x: 542,
				y: 85,
				width: 32 * 3,
			...
			uiBase.append(labelTekazu);

			this.setTekazu = (num: number) => {
				labelTekazu.text = "" + num;
				labelTekazu.invalidate();
			};

			//最短手数
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["score"], x: 505, y: 125, height: 32, srcY: 96 }));
			const labelSaitan = new g.Label({
				scene: this,
				x: 542,
				y: 125,
				width: 32 * 3,
			...
			uiBase.append(labelSaitan);

			this.setSaitan = (num: number) => {
				labelSaitan.text = "" + num;
				labelSaitan.invalidate();
			};

			//スコア
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["score"], x: 505, y: 165, height: 32 }));
			let score = 0;
